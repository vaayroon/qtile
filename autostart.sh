...
### Laptop + 2 external monitors (DP-3 from usc-c-hdmi adapter as secondary to the left,HDMI-1(attached to laptop) as primary on mid, eDP-1(Laptop) as thernary to the right) --> OFICIAL HOME INVERTED 1
#xrandr --output eDP-1 --mode 1920x1200 --pos 3840x0 --rotate normal --output HDMI-1 --primary --mode 1920x1080 --pos 1920x0 --rotate normal --output DP-1 --mode 1920x1080 --pos 0x0 --rotate normal --output DP-2 --off --output DP-3 --off --output DP-4 --off
...
### Layout is applied by Python hook (utils/system.py) with dynamic xrandr + fallback.
